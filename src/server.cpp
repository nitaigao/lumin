...
static void focus_view(turbo_view *view, struct wlr_surface *surface) {
...
static void server_new_keyboard(turbo_server *server, struct wlr_input_device *device) {
...
static void server_new_pointer(turbo_server *server, struct wlr_input_device *device) {
...
static bool view_at(turbo_view *view, double lx, double ly,
...
static turbo_view *desktop_view_at(turbo_server *server, double lx, double ly,
	struct wlr_surface **surface, double *sx, double *sy) {
...
static void process_cursor_motion(turbo_server *server, uint32_t time) {
	...
	turbo_view *view = desktop_view_at(server, server->cursor->x, server->cursor->y, &surface, &sx, &sy);
	if (!view) {
